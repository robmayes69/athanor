...
class CmdEvents(AthCommand):
    ...
    def switch_delete(self, lhs, rhs):
        if not lhs:
            self.error("No scene ID entered to delete.")
            return
        event = Event.objects.filter(id=lhs).first()
        if not event:
            self.error("Event not found.")
    ...
    def switch_desc(self, lhs, rhs):
        if not lhs:
            self.error("No scene ID entered to re-describe.")
            return
        event = Event.objects.filter(id=lhs).first()
        if not event:
            self.error("Event not found.")
        ...
        if not self.caller.is_admin or event.owner != self.character:
            ...
            return
        if not rhs:
            self.error("What will the new description be?")
            return
    ...
    def switch_title(self, lhs, rhs):
        if not lhs:
            self.error("No scene ID entered to re-title.")
            return
        event = Event.objects.filter(id=lhs).first()
        if not event:
            self.error("Event not found.")
        ...
        if not self.caller.is_admin or event.owner != self.character:
            ...
            return
        if not rhs:
            self.error("What will the new title be?")
            return
        event.title = rhs
        event.save()
    ...
    def switch_tag(self, lhs, rhs):
        ...
        if self.character in event.interest.all():
            self.error("You have already tagged that event.")
            return
    ...
    def switch_untag(self, lhs, rhs):
        ...
        if self.character not in event.interest.all():
            self.error("You have not tagged that event.")
            return
