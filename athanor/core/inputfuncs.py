...
from athanor.core.websocket import EchoRequest, LoginDetails, AccountGet, AccountAll, AccountDelete
from athanor.core.websocket import WhoGet, WhoAll, WhoLogout


def login_update(session, *args, **kwargs):
    request = LoginDetails(session, args, kwargs)
    request.process()


def account_all(session, *args, **kwargs):
    request = AccountAll(session, args, kwargs)
    request.process()


def account_get(session, *args, **kwargs):
    request = AccountGet(session, args, kwargs)
    request.process()


def account_delete(session, *args, **kwargs):
    request = AccountDelete(session, args, kwargs)
    request.process()

def who_get(session, *args, **kwargs):
    request = WhoGet(session, args, kwargs)
    request.process()

def who_all(session, *args, **kwargs):
    request = WhoAll(session, args, kwargs)
    request.process()

def who_logout(session, *args, **kwargs):
    request = WhoLogout(session, args, kwargs)
    request.process()