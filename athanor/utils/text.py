...
def clean_and_ansi(input_text: str, thing_name: str = "Name") -> (str, ANSIString):
    if not input_text:
        raise error.SyntaxException(f"{thing_name} must not be empty!")
    ...
    if '|' in clean_text:
        raise error.BadInputException(f"Malformed ANSI in {thing_name}.")
