...
class StatNodeHandler(MudHandler):
    category = 'stat'
    node_keys = dict()
    ...
    def load_nodes(self):
        """
        This loads up the stat nodes to the Class itself. The StatNode candidates are referenced on the Typeclass.

        Returns:
            None
        """
        candidates = self.owner.node_candidates
        for c in candidates:
            self.node_keys[c.key] = c
            if c.category is not None:
                if c.category not in self.node_categories:
                    self.node_categories[c.category] = list()
                self.node_categories[c.category].append(c)
            for t in c.tags:
                if t not in self.node_tags:
                    self.node_tags[t] = list()
                self.template_tags[t].append(t)
        for c in candidates:
            if c.parent_key in self.node_keys:
                self.node_keys[c.parent_key].children.append(c)
                c.parent_class = self.node_keys[c.parent_key]

    def load(self):
        if not self.node_loaded:
            self.load_nodes()
        self.save_data = {s.key: s.value for s in self.attr.get(category='stat', return_obj=True, return_list=True) if s}
        self.stat_keys = dict()
        for k in self.save_data:
            if k in self.node_keys:
                self.stat_keys[k] = self.node_keys[k](self)

    def __getitem__(self, item):
        if item in self.stat_keys:
            return self.stat_keys[item]
        else:
            if item in self.node_keys:
                load_stat = self.node_keys[item](self)
                self.stat_keys[item] = load_stat
                return load_stat
            else:
                return None