...
class BoardManager(AthanorSystem):
    ...
    def edit_post(self, session, board=None, post=None, seek_text=None, replace_text=None):
        board = self.find_board(session, board)
        posts = board.parse_postnums(session.account, post)
        if not posts:
            raise AthException("Post not found!")
        if len(posts) > 1:
            raise AthException("Cannot edit multiple posts at once.")
        p = posts[0]
        if not p.can_edit(session):
            raise AthException("Permission denied.")
        p.edit_post(find=seek_text, replace=replace_text)

    def delete_post(self, session, board=None, post=None):
        board = self.find_board(session, board)
        posts = board.parse_postnums(session.account, post)
        if not posts:
            raise AthException("Post not found!")
    ...
    def set_mandatory(self, session, board=None, value=None):
        board = self.find_board(session, board)
